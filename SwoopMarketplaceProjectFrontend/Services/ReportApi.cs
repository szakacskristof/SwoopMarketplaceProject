...
namespace SwoopMarketplaceProjectFrontend.Services
{
    public class ReportApi
    {
        ...
        .GetFromJsonAsync<List<ReportDto>>("api/Reports") ?? new();


        public async Task<ReportDto?> GetByAzonAsync(int azon)

        => await _f.CreateClient("SwoopApi")

        .GetFromJsonAsync<ReportDto>($"api/Reports/{azon}");
        ...
        public async Task UpdateAsync(int azon, ReportDto dto)

        {

            var r = await _f.CreateClient("SwoopApi")

            .PutAsJsonAsync($"api/Reports/{azon}", dto);
